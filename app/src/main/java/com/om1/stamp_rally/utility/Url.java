package com.om1.stamp_rally.utility;

/**
 * Created by b3176 on 2016/11/15.
 */

public final class Url {
    //IPアドレス
//    public static final String HOST = "35.160.83.150";    //AWS
//    public static final String HOST = "localhost";
//    public static final String HOST = "10.0.2.2";         //エミュレータ
    public static final String HOST = "192.168.2.101";  //ルーター
//    public static final String HOST = "192.168.137.1";  //水薮PC
//    public static final String HOST = "172.20.10.3";  //iphoneテザリング
    //ポート番号
//    public static final Integer PORT = 80;
    public static final Integer PORT = 8080;

    private Url(){}
}
